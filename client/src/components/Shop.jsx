    ...
    return (
        <div className="min-h-screen pt-24 pb-20 px-4 md:px-8 bg-transparent">
            <div className="container mx-auto">
                ...
                <div className="flex justify-between items-center lg:hidden mb-8 sticky top-20 z-30 bg-brand-surface/90 backdrop-blur-md p-4 rounded-xl shadow-lg border border-brand-light/10">
                    <button
                ...
                <div className="flex flex-col lg:flex-row gap-12 relative">
                    ...
                    `}>
                        <div className="flex justify-between items-center lg:hidden mb-8 border-b border-brand-light/10 pb-4">
                            <span className="font-serif text-2xl italic text-brand-light">Filters</span>
                            <button onClick={() => setIsFilterOpen(false)} className="p-2 -mr-2 hover:bg-brand-light/10 rounded-full text-brand-gray">
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                                ...
                                <div className="space-y-3 pt-4">
                                    {categories.map(cat => (
                                        <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                                            <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all duration-200 ${activeCategory === cat ? 'bg-brand-primary border-brand-primary' : 'border-brand-light/20 bg-brand-light/5 group-hover:border-brand-primary'}`}>
                                            ...
                                            />
                                            <span className={`text-sm tracking-wide transition-colors ${activeCategory === cat ? 'text-brand-primary font-bold' : 'text-brand-gray group-hover:text-brand-light'}`}>
                                                {cat}
                                            </span>
                                        </label>
                                    ))}
                            ...
                            <CollapsibleSection title="Price Range" defaultOpen={true}>
                                <div className="px-1 pt-6 pb-2">
                                    ...
                                    <div className="flex justify-between text-sm mt-4 font-medium text-brand-light">
                                        <span>₹0</span>
                                        <span className="bg-brand-light/5 px-2 py-1 rounded-md shadow-sm border border-brand-light/10">Max: ₹{priceRange.max}</span>
                                    </div>
                                </div>
                            </CollapsibleSection>
                        </div>
                    ...
                    <div className="flex-1">
                        ...
                        <div className="hidden lg:flex justify-between items-center mb-8 bg-brand-surface p-4 rounded-xl shadow-lg border border-brand-light/5">
                            <p className="text-brand-gray text-sm font-medium">Showing <span className="text-brand-light font-bold">{sortedProducts.length}</span> results</p>

                            <div className="relative">
                                <button
                                    onClick={() => setIsSortOpen(!isSortOpen)}
                                    className="flex items-center gap-2 text-sm uppercase tracking-wide font-bold text-brand-gray hover:text-brand-light"
                                >
                                ...
                                <AnimatePresence>
                                    {isSortOpen && (
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            exit={{ opacity: 0, y: 10 }}
                                            className="absolute top-full right-0 mt-4 w-56 bg-brand-surface shadow-xl z-20 border border-brand-light/10 rounded-xl overflow-hidden"
                                        >
                                            {sortOptions.map(option => (
                                                <button
                                                    key={option}
                        ...
                        {sortedProducts.length > 0 ? (
                            ...
                            <div className="text-center py-20 bg-brand-surface rounded-xl border border-brand-light/10 shadow-sm">
                                <p className="text-brand-gray mb-6 text-lg">No products found matching your filters.</p>
                                <button
...
const CollapsibleSection = ({ title, children, defaultOpen = false }) => {
    ...
    return (
        <div className="border-b border-brand-light/10 lg:border-none last:border-0 pb-6 lg:pb-0">
            <button
