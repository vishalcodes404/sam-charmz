        ...
        <AnimatePresence>
            {isVisible && (
                <motion.div
                    ...
                    className="fixed bottom-0 left-0 right-0 bg-brand-surface border-t border-brand-light/10 px-6 py-3 flex justify-between items-center z-40 lg:hidden shadow-[0_-5px_20px_rgba(0,0,0,0.5)] backdrop-blur-lg"
                >
                    {navItems.map((item) => {
                        ...
                        return (
                            <button
                                key={item.id}
                                onClick={item.action}
                                className={`flex flex-col items-center gap-1 relative transition-colors ${isActive ? 'text-brand-primary' : 'text-brand-gray hover:text-brand-light'}`}
                            >
